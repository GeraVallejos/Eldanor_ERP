...
function PresupuestosCreatePage() {
  ...
  const handleClienteSearchChange = (value) => {
    ...
    // While typing, clear stale selection until an option is chosen/resolved.
  ...
  const updateItemField = (index, key, value) => {
    setItems((prev) => {
      ...
      if (key === 'tipo') {
        ...
        if (value === 'SERVICIO' && !String(current.descripcion || '').trim()) {
          current.descripcion = ''
        }
      }
  ...
  const resolveClienteId = () => {
    ...
    if (filteredClientOptions.length === 1) {
  ...
  const onSubmit = async (event) => {
    ...
    let presupuestoCreadoId = null
    let creatingItemIndex = -1

    try {
      const { data: presupuesto } = await api.post(
        '/presupuestos/',
        {
      ...
      presupuestoCreadoId = presupuesto?.id
      ...
      for (let index = 0; index < items.length; index += 1) {
        ...
        await api.post(
          '/presupuesto-items/',
          {
            presupuesto: presupuestoCreadoId,
      ...
      if (presupuestoCreadoId) {
        try {
          await api.delete(`/presupuestos/${presupuestoCreadoId}/`, {
            suppressGlobalErrorToast: true,
          })
        } catch {
          // Si falla rollback, dejamos toast de error original y se puede limpiar manualmente.
        }
      }
    } finally {
      setStatus('idle')
    ...
    <section className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Crea un presupuesto en estado borrador.</p>
        </div>
        <Link
          to="/presupuestos"
      ...
      <form className="rounded-md border border-border bg-card p-4" onSubmit={onSubmit}>
        ...
        <div className="mt-5 rounded-lg border border-border p-3">
          ...
          {fieldErrors.items && (
            <p className="mb-2 text-xs text-destructive">{fieldErrors.items}</p>
          )}
        ...
        <div className="mt-4">
          <Button type="submit" size="md" disabled={status === 'loading'}>
            {status === 'loading' ? 'Guardando...' : 'Crear presupuesto'}
          </Button>
        </div>
      </form>
    </section>
...
export default PresupuestosCreatePage
