...
function InventarioTrasladosMasivosPage() {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true)
    try {
      const data = await inventarioApi.postOne(inventarioApi.endpoints.trasladosMasivos, {
        referencia,
  ...
  const handleDuplicate = async (documentoId) => {
    ...
    try {
      ...
      setLastDocument(data)
      await loadRecentDocuments(filters)
      toast.success('Traslado masivo duplicado correctamente.')
    } catch (error) {
  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h2 className="text-2xl font-semibold">Traslado masivo entre bodegas</h2>
          <p className="text-sm text-muted-foreground">Confirma un documento con multiples traslados entre la misma bodega origen y destino.</p>
        </div>
        <Link to="/inventario/traslados" className={cn(buttonVariants({ variant: 'outline', size: 'md' }))}>
          Volver a traslados simples
        </Link>
      </div>
      ...
      <div className="rounded-md border border-border bg-card p-4 space-y-4">
          ...
          {rows.map((row, index) => (
            <div key={row.id} className="grid gap-3 rounded-md border border-border p-3 md:grid-cols-12">
              <div className="md:col-span-8">
                <label className="text-sm">
                  Producto {index + 1}
                  <SearchableSelect className="mt-1" value={row.producto_id} onChange={(next) => updateRow(row.id, 'producto_id', next)} onSearchChange={(next) => { void searchProductos(next) }} options={productoOptions} ariaLabel={`Producto traslado masivo ${index + 1}`} placeholder="Buscar producto..." emptyText="No hay productos coincidentes" loading={loadingProductos} />
                </label>
              </div>
              <label className="text-sm md:col-span-3">
                Cantidad
                <input type="number" min="0" step="0.01" value={row.cantidad} onChange={(event) => updateRow(row.id, 'cantidad', event.target.value)} className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2" />
              </label>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button type="button" disabled={saving} onClick={handleSubmit}>
            {saving ? 'Confirmando...' : 'Confirmar traslado masivo'}
          </Button>
        </div>
      </div>
              ...
              <div key={documento.id} className="flex flex-col gap-2 rounded-md border border-border px-3 py-3 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {documento.bodega_origen_nombre || '-'} a {documento.bodega_destino_nombre || '-'} | {documento.items?.length || 0} lineas
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
